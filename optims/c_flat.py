...
class C_Flat(torch.optim.Optimizer):
    def __init__(self, params, base_optimizer, model, cflat=False, rho=0.2, lamb=0.2, adaptive=False, perturb_eps=1e-12,
                 grad_reduce='mean', strategy=None, A=5.0, k=0.01, t0=80, cof=0.005, args=None, **kwargs):
        ...
        if strategy is None and mode is not None:
            strategy = {
                "off": "off",
                "cflat": "basic",
                "cflat-plus": "plus",
            ...
            if strategy is None:
                raise ValueError(f'Invalid C-Flat mode: {mode}')
        if strategy is None:
            strategy = args.get("strategy")
        if strategy is None:
            strategy = "basic" if cflat else "off"
        if strategy not in {"off", "basic", "plus"}:
            raise ValueError(f'Invalid strategy: {strategy}')

        self.strategy = strategy
        self.cflat = strategy != "off"
        self.rho = args.get("rho", rho)
        self.lamb = args.get("lamb", lamb)
        self.A = self._config_value(args, "cflat_A", "A", A)
        self.k = self._config_value(args, "cflat_k", "k", k)
        self.t0 = self._config_value(args, "cflat_t0", "t0", t0)
        self.cof = self._config_value(args, "cflat_cof", "cof", cof)
        self._step_count = 0

    @staticmethod
    def _config_value(args, key, infty_key, default):
        value = args.get(key)
        return args.get(infty_key, default) if value is None else value
    ...
    def get_grad_l2_norm(self):
        ...
        return torch.norm(torch.cat(grad_list), p=2)
    ...
    def step(self, delay=False, closure=None):
        ...
        with self.maybe_no_sync():
            ...
            if self.strategy == "plus":
                grad_norm = self.get_grad_l2_norm()
                error = self.A / (1 + np.exp(-self.k * (self._step_count - self.t0))) - grad_norm
                self.A = self.A - self.cof * error
                if error <= 0:
